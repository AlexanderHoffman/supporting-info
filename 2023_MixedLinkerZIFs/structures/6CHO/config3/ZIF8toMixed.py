...
for i in range(len(num)):
    ...
    for j in range(len(linker_atoms)):
        ...
        if (all(elem in ring_atoms[group_rings[1,:],0:8] for elem in linker_atoms[j]) or all(elem in ring_atoms[group_rings[1,:],8:16] for elem in linker_atoms[j])):
            adapt = True
        print adapt
