...
for j in {E0,Ex,Ey,Ez};
    ...
    for k in {run1,run2,run3,run4};
        do
        if (([ "$j" = "Ey" ] && [ "$k" = "run1" ]) || ([ "$j" = "Ez" ] && [ "$k" = "run1" ])); then
            echo 'do it yourself'
            cat $j/dipole.txt $j/$k/combined.txt > $j/temp.txt; mv $j/temp.txt $j/dipole.txt
            wc -l < $j/dipole.txt
        elif [ -f $j/$k/moments.dat ]; then 
            grep 'X=' $j/$k/moments.dat | awk '{print $2,'t',$4,'t',$6}' > $j/$k/moments.txt 
            sed -n '2,2501p' $j/$k/moments.txt > $j/$k/moments_cut.txt
