#! /usr/bin/env python
# Script to calculate the Raman spectrum from a trajectory of polarizabilities

import numpy as np

# The molmod package (https://github.com/molmod/molmod) is used to define the units
...
#Load the trajectories of dipole moments without (E0) and with (Ex, Ey, Ez) the presence of an electric field
dip_E0 = np.genfromtxt('../E0/dipole.txt')[0:20000,:]
dip_Ex = np.genfromtxt('../Ex/dipole.txt')[0:20000,:]
dip_Ey = np.genfromtxt('../Ey/dipole.txt')[0:20000,:]
dip_Ez = np.genfromtxt('../Ez/dipole.txt')[0:20000,:]

Cell = np.genfromtxt('../cell.txt')

#Determine the trajectory of the polarizability tensor
alpha = polarizability(dip_E0,dip_Ex,dip_Ey,dip_Ez,cell=Cell)

#Calculate the Raman spectrum
freq, amps_para, amps_ortho = raman_spectrum(alpha,timestep=2.0*femtosecond,num_blocks=1,T=623.0*kelvin)
